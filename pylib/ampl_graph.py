...
class AmplGraph:
    ...
    def graph_resource(self):
        ...
        fig.update_layout(
            ...
            margin=dict(b=100), title_x=0.5
        )

        # Eliminar el título del eje X en todas las facetas
        for axis in fig.layout:
            if axis.startswith('xaxis'):
                fig.layout[axis].title.text = None  # Oculta los títulos del eje X

        # Ajustar los ejes Y
        # Oculta las marcas de graduación y líneas de los ejes Y excepto en el primer eje
        # Asumiendo que hay más de un escenario, ajustar según el número de escenarios
        num_scenarios = len(resources_per_year['Scenario'].unique())
        for i in range(2, num_scenarios + 1):  # Comienza en 2, ya que queremos mantener el primer eje Y
            fig.update_layout({
                f'yaxis{i}': {
                    'showticklabels': False,
                    'showline': False,
                    'ticks': ''  # Oculta las marcas de graduación
                }
            })

        # Asegurar que el primer eje Y muestre las etiquetas y la línea
        fig.layout['yaxis'].showticklabels = True
        fig.layout['yaxis'].showline = True

        # Añadir anotaciones correctamente
        for scenario in resources_per_year['Scenario'].unique():
            ...
            if total_2050 > 1:
                ...
                fig.add_annotation(dict(x=2050, y=total_2050, text=f"{total_2050:.2f}",
                                        ...
                                        showarrow=True, font=dict(size=11, color="black"),
                                        align="center", xanchor="center", yanchor="bottom"))

            # Ajustar las divisiones del eje X basándose en los valores de 'Years'
            fig.update_xaxes(tickvals=resources_per_year['Years'].unique())
        ...
        fig.update_layout(
            yaxis_title='Total cost (Billion of euros)',
            legend=dict(orientation="h", yanchor="bottom", y=-0.25, xanchor="center", x=0.5),
            margin=dict(b=100), title_x=0.5
        )
        ...
        for scenario in cost_per_year['Scenario'].unique():
            scenario_data = cost_per_year[(cost_per_year['Years'] == 2050) & (cost_per_year['Scenario'] == scenario)]
            total_2050 = scenario_data['Cost'].sum()
            ...
            # Ajustar las divisiones del eje X basándose en los valores de 'Years'
            fig.update_xaxes(tickvals=cost_per_year['Years'].unique())

        # Calcular la suma de costos por capa y dividir por 1000 para cada escenario
        for scenario in cost_per_year['Scenario'].unique():
            scenario_data = cost_per_year[cost_per_year['Scenario'] == scenario]
            ...
            fig.add_annotation(dict(
                ...
                text=f"Total public expenses<br>{annotation_text}",
    ...
    def graph_gwp_per_sector(self):
        ...
        layers = ['GAS','H2','AMMONIA','METHANOL','ELECTRICITY','HEAT_HIGH_T','PROCESS_COOLING','SPACE_COOLING','FOOD_PRESERVATION',
        ...
        # Reemplazar el valor 2021 por 2021 en la columna 'Years'
        gwp_per_layer['Years'] = gwp_per_layer['Years'].replace('2021', '2021')
        ...
        fig.update_layout(
            yaxis_title='Emissions (MtCO<sub>2eq.</sub>)',
            legend=dict(title_text='Sectors', orientation="v", yanchor="middle", y=0.5, x=0.9), title_x=0.5)
        ...
        for scenario in gwp_per_sector['Scenario'].unique():
            ...
            if total_2050 > 1:
                ...
                fig.add_annotation(dict(x=2050, y=total_2050, text=f"{total_2050:.2f}",
                                        ...
                                        align="center", xanchor="center", yanchor="bottom"))
            if scenario == 'CET':
                # Add a dashed horizontal line across all facets at the 2021 BAU total
                fig.add_hline(y=total_gwp_2021_bau, line_dash="dash", line_color="black", line_width=1, 
                            annotation_text=f"Year 2021: {total_gwp_2021_bau:.2f} MtCO<sub>2</sub>eq",
    ...
    def graph_tech_cap(self, ampl_collector=None, plot=True):
        ...
        for sector, tech in dict_tech.items():
            ...
            df_to_plot['Technologies'] = df_to_plot['Technologies'].astype("str")
            df_to_plot['Years'] = df_to_plot['Years'].str.replace('YEAR_', '')
            df_to_plot['Years'] = df_to_plot['Years'].replace('2021', '2021')
        ...
        for scenario in updated_data['Scenario'].unique():
            ...
            # Ajustar las divisiones del eje X basándose en los valores de 'Years'
            fig.update_xaxes(tickvals=updated_data['Years'].unique())

...
class AmplGraph:
    ...
    def __init__(self, pkl_file, ampl_obj,case_study):
        ...
        self.x_axis = [2021, 2025, 2030, 2035, 2040, 2045, 2050]
        # In 2021, 37.41% of electricity in EU-27 was produced from renewables (https://ec.europa.eu/eurostat/databrowser/view/NRG_IND_REN__custom_4442440/default/table?lang=en)
    ...
    def graph_resource(self):
        order_entry = ['ELECTRICITY','GASOLINE','DIESEL','LPG','LFO','COAL',
                       'GAS','URANIUM','WOOD','WET_BIOMASS','WASTE',
                       'RES_SOLAR','RES_WIND', 'RES_HYDRO','RES_GEO',
                       'AMMONIA_RE','METHANOL_RE','H2','H2_RE','GAS_RE']
        ...
        # Set the title and legend position
        fig.update_layout(title=dict(text="<b>Primary energy supply</b><br>[TWh]", x=0.5))
        fig.update_layout(legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.04))
        
        # Save the figure as HTML
        html_file_path_1 = self.outdir + "Resources.html"
        fig.write_html(html_file_path_1)
        
        # Also save as PDF if needed
        pdf_file_path = self.outdir + "Resources.pdf"
        fig.write_image(pdf_file_path, width=1200, height=550)

        file_path = self.outdir + "resources_per_year.csv"
        updated_data = df_to_plot
        
        # Guardar los datos actualizados en el archivo
        updated_data.to_csv(file_path, index=False)
        
        # Usar los datos actualizados para el gráfico
        df_to_plot = pd.read_csv(file_path)  # Asegúrate de que esta línea esté sincronizada con tus necesidades de datos

        # Crear el gráfico de barras apiladas
        fig = px.bar(df_to_plot, x='Years', y='Res', color='Resources',
                     title=None)  # Se establece title=None para ocultar el título
        
        # Cambiar el título del eje Y para usar subíndice en 'CO2eq' y ocultar el título del eje X
        fig.update_layout(
            yaxis_title='Energy (TWh)',
            xaxis_title=None
        )

        # Ajustar la posición de la leyenda para evitar colisiones
        fig.update_layout(legend=dict(orientation="h", yanchor="bottom", y=-0.225, xanchor="center", x=0.5))

        # Calcular el valor total de emisiones para el año 2050 y añadir la anotación
        total_2050 = df_to_plot[df_to_plot['Years'] == 2050]['Res'].sum()

        if total_2050 > 1:
            fig.add_annotation(x=2050, y=total_2050, text=f"{total_2050:.2f}", showarrow=True, font=dict(size=11, color="black"), align="center", xanchor="center", yanchor="bottom")
        
        # Ajustar las divisiones del eje X basándose en los valores de 'Years'
        fig.update_xaxes(tickvals=df_to_plot['Years'].unique())

        # Ajustes adicionales para mejorar la legibilidad
        fig.update_layout(margin=dict(b=100), title_x=0.5)  # Aumentar el margen inferior y centrar el título

        # Exportar el gráfico a HTML
        html_file_path = self.outdir + "Barplot_Resources_per_year.html"
        fig.write_html(html_file_path)

        # Exportar el gráfico a PDF
        pdf_file_path = self.outdir + "Barplot_Resources_per_year.pdf"
        fig.write_image(pdf_file_path, width=1200, height=550)

        file_path = self.outdir + "final_resources_per_year.csv"
        resources_per_year = pd.read_csv(file_path, sep=';')

        # Filtering out the year 2021 for CET and NZE scenarios
        resources_per_year = resources_per_year[~((resources_per_year['Years'] == 2021) & (resources_per_year['Scenario'].isin(['CET', 'NZE'])))]

        fig = px.bar(resources_per_year, x='Years', y='Res', color='Resources',
                    facet_col='Scenario', title=None)

        fig.update_layout(
            yaxis_title='Energy (TWh)',
            legend=dict(title_text='Sectors', orientation="v", yanchor="middle", y=0.625, x=1), title_x=0.5)

        # Hide the X-axis title on all facets
        for axis in fig.layout:
            if axis.startswith('xaxis'):
                fig.layout[axis].title.text = None

        # Adjusting Y-axes visibility
        num_scenarios = len(resources_per_year['Scenario'].unique())
        ...
        # Adjust bar width based on the maximum number of years in any scenario
        max_years = max(resources_per_year.groupby('Scenario')['Years'].nunique())
        standard_bar_width = 22 / max_years
        fig.update_traces(width=standard_bar_width)

        # Add annotations for the year 2050 and a horizontal line for the year 2021
        total_res_2021_bau = resources_per_year[(resources_per_year['Years'] == 2021) & (resources_per_year['Scenario'] == 'BAU')]['Res'].sum()

        for scenario in resources_per_year['Scenario'].unique():
            scenario_data = resources_per_year[(resources_per_year['Years'] == 2050) & (resources_per_year['Scenario'] == scenario)]
            total_2050 = scenario_data['Res'].sum()
            scenario_position = list(resources_per_year['Scenario'].unique()).index(scenario) + 1
            if total_2050 > 1:
                fig.add_annotation(dict(x=2050, y=total_2050, text=f"{total_2050:.2f}",
                                        xref=f"x{scenario_position}", yref=f"y",
                                        showarrow=True, font=dict(size=10, color="#404040"),
                                        align="center", xanchor="center", yanchor="bottom"))
            if scenario == 'NZE':
                fig.add_hline(y=total_res_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                            annotation_text=f"{total_res_2021_bau:.2f}", annotation_font=dict(color='#404040', size=10),
                            annotation_position="top left", row="all", col=scenario_position)
            else:
                fig.add_hline(y=total_res_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                            row="all", col=scenario_position)

        # Adjust X-axis ticks dynamically based on the available years in each scenario
        unique_years_per_scenario = resources_per_year.groupby('Scenario')['Years'].unique()
        for i, scenario in enumerate(resources_per_year['Scenario'].unique(), start=1):
            fig.update_xaxes(tickvals=unique_years_per_scenario[scenario], row=1, col=i)

        # Export options
        html_file_path = self.outdir + "final_barplot_resources_per_year.html"
        pdf_file_path = self.outdir + "final_barplot_resources_per_year.pdf"
        fig.write_html(html_file_path)
        fig.write_image(pdf_file_path, width=1200, height=550)

        file_path = self.outdir + "final_cost_per_year.csv"
        cost_per_year = pd.read_csv(file_path, sep=';')
        cost_per_year_all = cost_per_year
        # Filtering out the year 2021 for CET and NZE scenarios
        cost_per_year = cost_per_year[~((cost_per_year['Years'] == 2021) & (cost_per_year['Scenario'].isin(['CET', 'NZE'])))]
        
        fig = px.bar(cost_per_year, x='Years', y='Cost', color='Layers',
                    facet_col='Scenario', title=None)

        fig.update_layout(
            yaxis_title='Total cost (Million of euros)',
            legend=dict(title_text='Costs', orientation="v", yanchor="middle", y=0.625, x=1), title_x=0.5)
        ...
        # Adjusting Y-axes visibility
        num_scenarios = len(cost_per_year['Scenario'].unique())
        for i in range(2, num_scenarios + 1):
            fig.update_layout({
                f'yaxis{i}': {
                    'showticklabels': False,
        ...
        # Adjust bar width based on the maximum number of years in any scenario
        max_years = max(cost_per_year.groupby('Scenario')['Years'].nunique())
        standard_bar_width = 22 / max_years
        fig.update_traces(width=standard_bar_width)
        
        # Calcular la suma de costos por capa y dividir por 1000 para cada escenario
        for scenario in cost_per_year_all['Scenario'].unique():
            scenario_data = cost_per_year_all[cost_per_year_all['Scenario'] == scenario]
            # Calcular la suma de costos por capa y convertir a millones de euros
            total_costs_per_layer = scenario_data.groupby('Layers')['Cost'].sum() / 1000
            scenario_position = list(cost_per_year_all['Scenario'].unique()).index(scenario) + 1
            
            # Preparar el texto de la anotación concatenando cada total de gastos por capa con un salto de línea
            annotation_text = "<br>".join([f"{layer}: {total_cost:.2f} billion €" for layer, total_cost in total_costs_per_layer.items()])
            
            # Añadir una única anotación por escenario con todos los totales de gastos por capa
            fig.add_annotation(dict(
                x=0,  # Posición x al inicio del gráfico
                y=1,  # Posición y en la parte superior del gráfico
                text=f"Total expenses<br>{annotation_text}",
                xref=f"x{scenario_position} domain" if scenario_position > 1 else "x domain",
                yref=f"y{scenario_position} domain" if scenario_position > 1 else "y domain",
                showarrow=False,
                font=dict(size=11),
                align="left",
                xanchor="left",
        ...
        # Calculate the total cost for the year 2021 in the BAU scenario and add a dashed line and annotation for NZE
        total_cost_2021_bau = cost_per_year[(cost_per_year['Years'] == 2021) & (cost_per_year['Scenario'] == 'BAU')]['Cost'].sum()
        
        for scenario in cost_per_year['Scenario'].unique():
            scenario_position = list(cost_per_year['Scenario'].unique()).index(scenario) + 1
            if scenario == 'NZE':
                fig.add_hline(y=total_cost_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                              annotation_text=f"Year 2021: {(total_cost_2021_bau)/1000:.2f} billion €", annotation_font=dict(color='#404040', size=10),
                              annotation_position="top left", row="all", col=scenario_position)
            else:
                fig.add_hline(y=total_cost_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                              row="all", col=scenario_position)
        ...
        # Adjust X-axis ticks dynamically based on the available years in each scenario
        unique_years_per_scenario = cost_per_year.groupby('Scenario')['Years'].unique()
        for i, scenario in enumerate(cost_per_year['Scenario'].unique(), start=1):
            fig.update_xaxes(tickvals=unique_years_per_scenario[scenario], row=1, col=i)

        html_file_path = self.outdir + "final_barplot_cost_per_year.html"
        pdf_file_path = self.outdir + "final_barplot_cost_per_year.pdf"
        fig.write_html(html_file_path)
        fig.write_image(pdf_file_path, width=1200, height=550)
        
        return html_file_path_1  # Return the path to the generated HTML file
    ...
    def graph_gwp_per_sector(self):
        ...
        layers = ['GAS','H2','AMMONIA','METHANOL','ELECTRICITY','DIESEL','GASOLINE','LFO','LPG','HEAT_HIGH_T','PROCESS_COOLING','SPACE_COOLING','FOOD_PRESERVATION',
                'HEAT_LOW_T_DECEN','HEAT_LOW_T_DHN','HVC',
                'MOB_FREIGHT_BOAT','MOB_FREIGHT_RAIL','MOB_FREIGHT_ROAD','MOB_PRIVATE',
                'MOB_PUBLIC', 'MOB_FREIGHT_AIR', 'COOKING', 'MECHANICAL_ENERGY_COMM', 'MECHANICAL_ENERGY_IND', 'MECHANICAL_ENERGY_MOV_AGR', 'MECHANICAL_ENERGY_FIX_AGR', 'MECHANICAL_ENERGY_MIN', 'MECHANICAL_ENERGY_FISH_OTHERS']
        mob_freight_layers = ['MOB_FREIGHT_BOAT','MOB_FREIGHT_ROAD','MOB_FREIGHT_RAIL','MOB_FREIGHT_AIR']
        mob_mechanical_energy_layers = ['MECHANICAL_ENERGY_COMM', 'MECHANICAL_ENERGY_IND', 'MECHANICAL_ENERGY_MOV_AGR', 'MECHANICAL_ENERGY_FIX_AGR', 'MECHANICAL_ENERGY_MIN', 'MECHANICAL_ENERGY_FISH_OTHERS']
        cooling_ref_layers = ['PROCESS_COOLING','SPACE_COOLING','FOOD_PRESERVATION']
        ...
        order_entry = ['MOB_PRIVATE','MOB_PUBLIC','MOBILITY_FREIGHT', 'ELECTRICITY',
                       ...
                       'HVC','METHANOL','AMMONIA','COOKING', 'MECHANICAL_ENERGY']
        ...
        update_gwp_res = ['GAS','H2','AMMONIA','ELECTRICITY','DIESEL','GASOLINE','METHANOL','LFO','LPG']
        ...
        mob_mechanical_energy = gwp_per_layer.loc[gwp_per_layer.index.get_level_values('Layers').isin(mob_mechanical_energy_layers)]
        mob_mechanical_energy = mob_mechanical_energy.groupby(['Years']).sum()
        mob_mechanical_energy.reset_index(inplace=True)
        mob_mechanical_energy['Layers'] = 'MECHANICAL_ENERGY'
        mob_mechanical_energy.set_index(['Years','Layers'],inplace=True)
        others = gwp_per_layer.loc[~gwp_per_layer.index.get_level_values('Layers').isin(mob_mechanical_energy_layers)]
        gwp_per_layer = pd.concat([gwp_per_layer,mob_mechanical_energy])

        cooling_ref = gwp_per_layer.loc[gwp_per_layer.index.get_level_values('Layers').isin(cooling_ref_layers)]
        cooling_ref = cooling_ref.groupby(['Years']).sum()
        cooling_ref.reset_index(inplace=True)
        cooling_ref['Layers'] = 'COOLING & REFRIGERATION'
        cooling_ref.set_index(['Years','Layers'],inplace=True)
        others = gwp_per_layer.loc[~gwp_per_layer.index.get_level_values('Layers').isin(cooling_ref_layers)]
        gwp_per_layer = pd.concat([gwp_per_layer,cooling_ref])
        ...
        # Limpieza de datos para asegurarse de que no hay filas con valores NaN o cadenas vacías que puedan causar el error
        gwp_per_layer.dropna(subset=['Years', 'GWP_EUD', 'Layers'], inplace=True)

        # Asegurar que 'Years' es un tipo de dato correcto para el reemplazo
        gwp_per_layer['Years'] = gwp_per_layer['Years'].astype(str)

        # Configuraciones previas del gráfico (ya incluidas en tu código)
        ...
        new_title = "<b>Yearly emissions</b><br>[MtCO2/y]"
        fig.update_layout(title=new_title)

        temp = gwp_per_layer.drop(columns=['Layers']).groupby('Years').sum()
        # yvals = [0,max(list(map(int,temp['GWP_EUD'])))]
        
        # # Ajustar los límites del eje Y del gráfico
        # fig.update_layout(yaxis_range=yvals)

        # Paso 1: Calcular el valor máximo de GWP_EUD
        max_gwp_eud = temp['GWP_EUD'].max()

        # Paso 2 y 3: Establecer el rango del eje Y con un margen adicional
        # Aquí, agregamos un 10% más al valor máximo para el margen
        y_axis_max = max_gwp_eud * 1.10  # Ajusta el 1.10 según necesites para el margen

        # Ajustar los límites del eje Y del gráfico
        fig.update_layout(yaxis_range=[0, y_axis_max])

        # Guardar el gráfico como HTML
        html_file_path = self.outdir + "Gwp_per_sector.html"
        fig.write_html(html_file_path)

        # Guardar el gráfico como PDF
        pdf_file_path = self.outdir + "Gwp_per_sector.pdf"
        fig.write_image(pdf_file_path, width=1200, height=550)
        
        file_path = self.outdir + "gwp_per_sector.csv"
        updated_data = gwp_per_layer
        
        # Guardar los datos actualizados en el archivo
        updated_data.to_csv(file_path, index=False)
        
        # Usar los datos actualizados para el gráfico
        gwp_per_layer = pd.read_csv(file_path)  # Asegúrate de que esta línea esté sincronizada con tus necesidades de datos

        # Crear el gráfico de barras apiladas
        fig = px.bar(gwp_per_layer, x='Years', y='GWP_EUD', color='Layers',
                     title=None)  # Se establece title=None para ocultar el título
        
        # Cambiar el título del eje Y para usar subíndice en 'CO2eq' y ocultar el título del eje X
        fig.update_layout(
            yaxis_title='Emissions (MtCO<sub>2eq.</sub>)',
            xaxis_title=None
        )
        ...
        # Calcular el valor total de emisiones para el año 2050 y añadir la anotación
        total_2050 = gwp_per_layer[gwp_per_layer['Years'] == 2050]['GWP_EUD'].sum()

        if total_2050 > 1:
            fig.add_annotation(x=2050, y=total_2050, text=f"{total_2050:.2f}", showarrow=True, font=dict(size=11, color="black"), align="center", xanchor="center", yanchor="bottom")
        
        # Ajustar las divisiones del eje X basándose en los valores de 'Years'
        fig.update_xaxes(tickvals=gwp_per_layer['Years'].unique())

        # Ajustes adicionales para mejorar la legibilidad
        fig.update_layout(margin=dict(b=100), title_x=0.5)  # Aumentar el margen inferior y centrar el título

        # Exportar el gráfico a HTML
        html_file_path = self.outdir + "Barplot_GWP_per_sector.html"
        fig.write_html(html_file_path)

        # Exportar el gráfico a PDF
        pdf_file_path = self.outdir + "Barplot_GWP_per_sector.pdf"
        fig.write_image(pdf_file_path, width=1200, height=550)

        file_path = self.outdir + "final_gwp_per_sector.csv"
        gwp_per_sector = pd.read_csv(file_path, sep=';')

        # Filter out the year 2021 for CET and NZE scenarios
        gwp_per_sector = gwp_per_sector[~((gwp_per_sector['Years'] == 2021) & (gwp_per_sector['Scenario'].isin(['CET', 'NZE'])))]

        fig = px.bar(gwp_per_sector, x='Years', y='GWP_EUD', color='Layers',
                    facet_col='Scenario', title=None,
                    barmode='stack')

        # Find the minimum number of years in any scenario to calculate the bar width
        max_years = max(gwp_per_sector.groupby('Scenario')['Years'].nunique())
        # Standard bar width across facets, adjust the 0.8 scale factor as needed for visual spacing
        standard_bar_width = 22 / max_years

        fig.update_traces(width=standard_bar_width)  # Setting the same bar width across all scenarios
        
        fig.update_layout(
            yaxis_title='Emissions (MtCO<sub>2eq.</sub>)',
            legend=dict(title_text='Sectors', orientation="v", yanchor="middle", y=0.625, x=0.99), title_x=0.5)
        ...
        # Adjust Y-axes visibility
        num_scenarios = len(gwp_per_sector['Scenario'].unique())
        for i in range(2, num_scenarios + 1):
            fig.update_layout({
                f'yaxis{i}': {
                    'showticklabels': False,
        ...
        # Calculate the total GWP_EUD for the year 2021 in the BAU scenario
        total_gwp_2021_bau = gwp_per_sector[(gwp_per_sector['Years'] == 2021) & (gwp_per_sector['Scenario'] == 'BAU')]['GWP_EUD'].sum()

        # Adding annotations correctly
        for scenario in gwp_per_sector['Scenario'].unique():
            scenario_data = gwp_per_sector[(gwp_per_sector['Years'] == 2050) & (gwp_per_sector['Scenario'] == scenario)]
            total_2050 = scenario_data['GWP_EUD'].sum()
            scenario_position = list(gwp_per_sector['Scenario'].unique()).index(scenario) + 1
            if total_2050 > 1:
                fig.add_annotation(dict(x=2050, y=total_2050, text=f"{total_2050:.2f}",
                                        xref=f"x{scenario_position}", yref=f"y",
                                        showarrow=True, font=dict(size=10, color="#404040"),
                                        align="center", xanchor="center", yanchor="bottom", arrowcolor="#404040", ay=-25))
            if scenario == 'NZE':
                fig.add_hline(y=total_gwp_2021_bau, line_dash="dash", line_color="#000000", line_width=1.2, opacity=1, 
                              annotation_text=f"Year 2021: {total_gwp_2021_bau:.2f}", annotation_font=dict(color='#404040', size=10),
                              annotation_position="top right", row="all", col=scenario_position)
            else:
                fig.add_hline(y=total_gwp_2021_bau, line_dash="dash", line_color="#000000", line_width=1.2, opacity=1, 
                              row="all", col=scenario_position)
                            
        # Adjust X-axis ticks dynamically based on the available years in each scenario
        unique_years_per_scenario = gwp_per_sector.groupby('Scenario')['Years'].unique()
        for i, scenario in enumerate(gwp_per_sector['Scenario'].unique(), start=1):
            fig.update_xaxes(tickvals=unique_years_per_scenario[scenario], row=1, col=i)

        html_file_path = self.outdir + "final_barplot_GWP_per_scenario.html"
        pdf_file_path = self.outdir + "final_barplot_GWP_per_scenario.pdf"
        fig.write_html(html_file_path)
        fig.write_image(pdf_file_path, width=1200, height=550)
    ...
    def graph_cost(self):
        ...
        for i,j in enumerate(['2021','2025','2030','2035','2040','2045','2050']):
        ...
        yvals = [0,min(round(temp['Cost'],1)),round(temp.loc['2021']['Cost'],1),max(round(temp['Cost'],1))]
    ...
    def graph_cost_return(self):
        ...
        cost_return_eff = cost_return_eff[~cost_return_eff['Years'].isin(['2021','2025'])]
        ...
        c_inv_cum = c_inv_cum[~c_inv_cum['Years'].isin(['2021','2025'])]
    ...
    def graph_cost_inv_phase_tech(self, ampl_collector = None, plot=True):
        ...
        df_to_plot = df_to_plot.loc[~df_to_plot['Phases'].isin(['2015_2021'])]
        df_to_plot['Technologies'] = df_to_plot['Technologies'].astype("str")
    ...
    def graph_cost_op_phase(self, ampl_collector = None, plot=True):
        ...
        results_2021 = ampl_collector['Cost_breakdown'].copy()
        results_2021 = results_2021.loc[results_2021.index.get_level_values('Years') == 'YEAR_2021',:]
        results_2021.fillna(0,inplace=True)
        results_2021['C_op'] = results_2021['C_op']+results_2021['C_maint']
        results_2021.drop(columns=['C_inv','C_maint'],inplace=True)
        results_2021.reset_index(inplace=True)
        results_2021['Years'] = results_2021['Years'].map(lambda x: x[-4:])
        
        df_to_plot = pd.concat([results_2021,results_tech,results_res],axis=0,ignore_index=True)
    ...
    def graph_tech_cap(self, ampl_collector=None, plot=True):
        ...
        for sector, tech in dict_tech.items():
            ...
            # Guardar df_to_plot en un archivo CSV
            csv_file_path = Path(self.outdir + "Tech_Cap/" + sector + ".csv")
            ...
            if plot:
                ...
                # Ajustes de diseño similares a `graph_resource`
                fig.update_layout(title=dict(text="<b>" + self.case_study + " - " + sector + "</b><br>[Installed capacity]", x=0.5))
                fig.update_layout(legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.04))

                outdir_path = Path(self.outdir + "Tech_Cap/")
                outdir_path.mkdir(parents=True, exist_ok=True)

                # Guardar como HTML
                html_file_path = str(outdir_path / (sector + ".html"))
                fig.write_html(html_file_path)

                # Guardar como PDF
                pdf_file_path = str(outdir_path / (sector + ".pdf"))
                fig.write_image(pdf_file_path, width=1200, height=550)

            if df_to_plot_full.empty:
                df_to_plot_full = df_to_plot
            else:
                df_to_plot_full = pd.concat([df_to_plot_full, df_to_plot], ignore_index=True)

        # Leer el archivo final_tech_cap.csv existente y actualizarlo con df_to_plot_full
        file_path = Path(self.outdir + "final_tech_cap.csv")

        # Leer los datos actualizados para imprimir los escenarios
        updated_data = pd.read_csv(file_path, sep=';')
        
        # Filtering out the year 2021 for specific scenarios
        updated_data = updated_data[~((updated_data['Years'] == 2021) & (updated_data['Scenario'].isin(['CET', 'NZE'])))]
        
        fig = px.bar(updated_data, x='Years', y='F', color='Technologies',
                     facet_col='Scenario', title=None)
        ...
        # Adjust Y-axes visibility
        num_scenarios = len(updated_data['Scenario'].unique())
        for i in range(2, num_scenarios + 1):
            fig.update_layout({
                f'yaxis{i}': {
                    'showticklabels': False,
        ...
        # Adjust bar width based on the maximum number of years in any scenario
        max_years = max(updated_data.groupby('Scenario')['Years'].nunique())
        standard_bar_width = 22 / max_years
        fig.update_traces(width=standard_bar_width)

        # Calculate the total for the year 2021 in the BAU scenario
        total_2021_bau = updated_data[(updated_data['Years'] == 2021) & (updated_data['Scenario'] == 'BAU')]['F'].sum()

        for scenario in updated_data['Scenario'].unique():
            scenario_position = list(updated_data['Scenario'].unique()).index(scenario) + 1
            if scenario == 'NZE':
                fig.add_hline(y=total_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                              annotation_text=f"{total_2021_bau:.2f}", annotation_font=dict(color='#404040', size=10),
                              annotation_position="top left", row="all", col=scenario_position)
            else:
                fig.add_hline(y=total_2021_bau, line_dash="dash", line_color="#404040", line_width=1.2, opacity=1, 
                              row="all", col=scenario_position)

            scenario_data = updated_data[(updated_data['Years'] == 2050) & (updated_data['Scenario'] == scenario)]
            total_2050 = scenario_data['F'].sum()
            if total_2050 > 1:
                scenario_position = list(updated_data['Scenario'].unique()).index(scenario) + 1
                fig.add_annotation(dict(x=2050, y=total_2050, text=f"{total_2050:.2f}",
                                        xref=f"x{scenario_position}", yref=f"y",
        ...
        # Adjust X-axis ticks dynamically based on the available years in each scenario
        unique_years_per_scenario = updated_data.groupby('Scenario')['Years'].unique()
        for i, scenario in enumerate(updated_data['Scenario'].unique(), start=1):
            fig.update_xaxes(tickvals=unique_years_per_scenario[scenario], row=1, col=i)

        html_file_path = self.outdir + "final_barplot_tech_cap.html"
        pdf_file_path = self.outdir + "final_barplot_tech_cap.pdf"
        fig.write_html(html_file_path)
        fig.write_image(pdf_file_path, width=1200, height=550)
    ...
    def _get_cost_return_for_each_year(self):
        ...
        phases = ['2015_2021'] + self.ampl_obj.sets['PHASE']
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            ...
            color_dict = {'YEAR_2021': 'blue','YEAR_2025': 'orange','YEAR_2030': 'green', 'YEAR_2035': 'red', 'YEAR_2040': 'purple', 'YEAR_2045': 'brown', 'YEAR_2050':'pink'}
        elif category == 'Phases':
                color_dict = {'2015_2021': 'blue','2021_2025': 'orange','2025_2030': 'green', '2030_2035': 'red', '2035_2040': 'purple', '2040_2045': 'brown', '2045_2050':'pink'}
            
        return color_dict
    
    
    @staticmethod
    def custom_fig(fig,title,yvals,xvals=['2021','2025','2030','2035','2040',

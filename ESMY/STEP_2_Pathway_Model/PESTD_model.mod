...
#NEW DEPENDENT:
set AGE {TECHNOLOGIES,PHASE} within PHASE union {"2015_2021"} union {"STILL_IN_USE"};



## NEW SETS FOR ANTERIOR PHASES TO THE OPTIMIZATION WINDOW
set ANT_INSTALL_PERIODS; # Anterior phases to the optimization window
...
param decom_allowed {PHASE,PHASE union {"2015_2021"},TECHNOLOGIES} default 0;
...
param share_freight_air_min {YEARS}  >= 0, <= 1 default 0; # % min limit for penetration of aircraft in freight transportation
param share_freight_air_max {YEARS}  >= 0, <= 1 default 0; # % max limit for penetration of aircraft in freight transportation
...
## NEW PARAMETERS FOR ANTERIOR PHASES TO THE OPTIMIZATION WINDOW
param ant_install_year {ANT_INSTALL_PERIODS} default 0; # Start year for each installation period
param ant_install_capacity {ANT_TECHNOLOGIES, ANT_INSTALL_PERIODS} default 0; # Installed capacity by technology and installation period
param ant_technology_lifespan {ANT_TECHNOLOGIES, ANT_INSTALL_PERIODS} default 25; # Technology lifetime per installation period
...
var F_new {PHASE union {"2015_2021"}, TECHNOLOGIES} >= 0; #[GW/GWh] Accounts for the additional new capacity installed in a new phase
var F_decom {PHASE,PHASE union {"2015_2021"}, TECHNOLOGIES} >= 0; #[GW] Accounts for the decommissioned capacity in a new phase
...
var Share_freight_air {y in YEARS}, >= share_freight_air_min [y], <= share_freight_air_max [y]; # %_Air: % of freight mobility attributed to boat
...
## NEW VARIABLES FOR ANTERIOR PHASES TO THE OPTIMIZATION WINDOW
...
subject to end_uses_t {y in YEARS_WND diff YEAR_ONE, l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [y,l, h, td] = (if l == "ELECTRICITY" 
		then
			...
			(end_uses_input[y,"LIGHTING_P"] * electricity_time_series [h, td] / t_op [h, td])			
			...
			(end_uses_input[y,"MOBILITY_FREIGHT"]   * mob_freight_time_series [h, td] / t_op [h, td] ) *  Share_freight_air [y]
...
subject to totalGWPTransition_calculation : # category: GWP_calc
	TotalGWPTransition = TotalGWP ["YEAR_2021"] + sum {p in PHASE_UP_TO union PHASE_WND,y_start in PHASE_START [p],y_stop in PHASE_STOP [p]}  (t_phase [p] * (TotalGWP [y_start] + TotalGWP [y_stop])/2);
...
subject to Freight_shares {y in YEARS_WND diff YEAR_ONE} :
	Share_freight_train [y] + Share_freight_road [y] + Share_freight_boat [y] + Share_freight_air [y] = 1;
...
subject to phase_new_build {p in PHASE_WND, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in TECHNOLOGIES}:
	F[y_stop,i] = F[y_start,i] + F_new[p,i] - F_old [p,i] 
  											     - sum {p2 in {PHASE_WND union PHASE_UP_TO union {"2015_2021"}}} F_decom[p,p2,i];
...
subject to define_f_decom_properly {p_decom in PHASE, p_built in PHASE union {"2015_2021"}, i in TECHNOLOGIES}:
	if decom_allowed[p_decom,p_built,i] == 0 then F_decom [p_decom,p_built,i] = 0;

# [Eq. XX] Intialise the first phase based on YEAR_2021 results
subject to F_new_initialisation {tech in TECHNOLOGIES}:
	F_new ["2015_2021",tech] = F["YEAR_2021",tech]; # Generate F_new2015_2021

subject to no_decom_if_no_built {i in TECHNOLOGIES, p in PHASE_WND union PHASE_UP_TO union {"2015_2021"}}:
...
subject to investment_computation_tech {p in PHASE_WND union PHASE_UP_TO union {"2015_2021"}, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in TECHNOLOGIES}:
...
subject to investment_return {i in TECHNOLOGIES}:
	C_inv_return [i] = sum {p in PHASE_WND union PHASE_UP_TO union {"2015_2021"},y_start in PHASE_START [p],y_stop in PHASE_STOP [p]} 
...
subject to Opex_tot_cost_calculation :# category: COST_calc
	C_tot_opex = C_opex["YEAR_2021"] 
				 + sum {p in PHASE_WND union PHASE_UP_TO,y_start in PHASE_START [p],y_stop in PHASE_STOP [p]} ( 
					                 t_phase [p] * (C_opex [y_start] + C_opex [y_stop])/2 * annualised_factor[p] ); #In euros_2015
...
subject to operation_computation_tech {p in PHASE_WND union PHASE_UP_TO, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in TECHNOLOGIES}:
	C_op_phase_tech [p,i] = t_phase [p] *   ((C_maint [y_start,i] + C_maint [y_stop,i])/2 *annualised_factor[p] ); #In euros_2015
...
subject to operation_computation_res {p in PHASE_WND union PHASE_UP_TO, y_start in PHASE_START[p], y_stop in PHASE_STOP[p], i in RESOURCES}:
	C_op_phase_res [p,i] = t_phase [p] *   ((C_op [y_start,i] + C_op [y_stop,i])/2 *annualised_factor[p] ); #In euros_2015
...
subject to Gwp_tot_cost_calculation:
	Gwp_tot_cost = sum {p in PHASE_WND union PHASE_UP_TO,y_start in PHASE_START [p],y_stop in PHASE_STOP [p]} ( 
					                 t_phase [p] * (TotalGWP [y_start]*gwp_cost[y_start] + TotalGWP [y_stop]*gwp_cost[y_stop])/2);

...
var Share_freight_road {y in YEARS}, >= share_freight_road_min [y], <= share_freight_road_max [y]; # %_Road: % of freight mobility attributed to road
var Share_freight_boat {y in YEARS}, >= share_freight_boat_min [y], <= share_freight_boat_max [y]; # %_Boat: % of freight mobility attributed to boat
...
subject to end_uses_t {y in YEARS_WND diff YEAR_ONE, l in LAYERS, t in PERIODS}:
	End_uses [y,l, t] = (if l == "ELECTRICITY" 
		then
			(end_uses_input[y,l] / total_time) + Network_losses [y,l,t]
		else (if l == "LIGHTING_R_C" then
			(end_uses_input[y,l] * lighting_month [t] / t_op [t])
			...
			(end_uses_input[y,"MOBILITY_FREIGHT"] / total_time) * Share_freight_air [y]			
